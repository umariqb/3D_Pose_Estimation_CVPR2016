function trajectories = C_recursive_forwardKinematicsQuat(skel, mot, node_id, current_position, current_rotation, trajectories)
...
for child_id = skel.nodes(node_id).children'
    ...
    if (~isempty(mot.rotationQuat{child_id}))
        child_rotation = C_quatmult(current_rotation,mot.rotationQuat{child_id});
    else
        child_rotation = current_rotation;
    end

    child_position = current_position + C_quatrot(child.offset,child_rotation);

    trajectories = C_recursive_forwardKinematicsQuat(skel, mot, child_id, child_position, child_rotation, trajectories);
end