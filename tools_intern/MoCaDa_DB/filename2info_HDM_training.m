function [info,OK] = filename2info_HDM_training(amcfullpath)
info = struct('amcname','',...
              'asfname','',...
              'amcpath','',...
              'filetype','ASF/AMC',...
...
amcfullpath=convertfilenames(amcfullpath);
...
if (length(p)~=4)
    disp(['**** Filename "' info.amcname '" doesn''t conform with MoCaDa standard! (filename2info_HDM_training)']);
    OK = false;
    return;
end
...
[n,info.motionClass] = motion_category_get_numreps(info.motionCategory);
info.numReps = n;
