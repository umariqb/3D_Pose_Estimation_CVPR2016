function f = objfunNX(x,optimStruct)
    ...
    case {'exp','ExpMap'}
        for j=1:motRec.njoints
            if(~isempty(motRec.rotationQuat{j}))
                motRec.rotationExpMap{j}=quatlog(motRec.rotationQuat{j});
            end
        end
...
P=zeros(size(optimStruct.originalTraj{1},1),numJoints*numFrames);
Q=zeros(size(optimStruct.originalTraj{1},1),numJoints*numFrames);
