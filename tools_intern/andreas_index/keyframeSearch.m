...
%         end
    ...
    if (nargout > 4)
        ...
        listLength = sum(bucketsSizes, 1);
        varargout{3} = listLength;
    end
...
%     files_frame_length = 150;
    %Erwartetes Ergebis: Segment: Doc 1, start: 1, end: 36
...
% 
%     keyframeBuckets{1, 4} = [4,4, 5,5, 6,6, 7,7];
% 
%     keyframeDistance = [1, 1, 1];
%     stiffness = 1.0;
%     distanceMinMax = ([stiffness; 1/stiffness]*keyframeDistance)';
%     distanceMinMax(:,1) = ceil(distanceMinMax(:,1));
%     distanceMinMax(:,2) = floor(distanceMinMax(:,2));
...
%     K = 3;
%     keyframeBuckets = cell(files_num, K);
%     keyframeBuckets{1, 1} = [50,50];
%     
%     keyframeBuckets{1, 2} = [60,60, 70,70, 80,80];
% 
%     keyframeBuckets{1, 3} = [40,40, 60,60, 100,100];
% 
%     %keyframeBuckets{1, 4} = [4,4, 5,5, 6,6, 7,7];
% 
%     %keyframeDistance = [1, 1, 1];
%     distanceMinMax = [0, inf; 0,inf];
%     
%     extendLeft = 5;
%     extendRight = 10;
%     files_frame_length = 150;
%       %  erwartetes Ergebnis: 1, 45, 110
    ...
    if (nargout > 5)
        %measure the runtime for the recursive function
        tic;
    end
    ...
    for i = 2:hitsNum;
       segmentsNum = segmentsNum + 1;
        ...
        if overlapping
            ...
            %remove the matching frames of the hits that are merged into
            %this one to have an exact correspondence of hit and
            %matchingFrames.
            %Of course this deletes information, hit relevant ranges
            %of matches overlap
...
function keyframeSequenceFindRecursive(docID, keyframeNr, allowedRange)
    ...
    if (keyframeNr == K)
        ...
        while keyframePointer(K) <= size(keyframes, 2) ...
            ...
            %Berechne die matchingFrames. Das sind für jeden
            %Keyframe die größten Intervalle, die von einem Keyframe
            %gematcht wurden. Dafür starte die Reconstruction of hits
            %einmal mit dem linken Intervallrand der intersection des K-ten
            %Keyframes und einmal mit dem rechten Intervallrand der
            %intersection des K-ten Keyframes. Danach werden die Intervalle
            %vereinigt und man erhält die größtmöglichen gematchten
            %Bereiche.
            
            index(K) = intersection{K}(2); %starte Reconstruction mit rechtestem Wert der IntersectionK
            matchingFrames{hitsNum} = zeros(K, 2);
            matchingFrames{hitsNum}(K,2) = intersection{K}(2);
            for k=K-1:-1:1
                ...
                %speichere nur die rechte Intervallgrenze
                matchingFrames{hitsNum}(k,2)=J(2);
                
                index(k) = J(2);
            end
            
            index(K) = intersection{K}(1); %starte Reconstruction mit linkesten Wert der IntersectionK
            matchingFrames{hitsNum}(K,1) = intersection{K}(1);
            for k=K-1:-1:1
                I=[index(k+1), index(k+1)] - fliplr(distanceMinMax(k,:));
                J=[max(I(1), intersection{k}(1)),...
                    min(I(2), intersection{k}(2))];
                ...
                %speichere nur die linke Intervallgrenze
                matchingFrames{hitsNum}(k,1)=J(1);
                
                index(k) = J(1);
            end
            
            
            hits(3, hitsNum) = intersection{K}(2); %der größte Wert aller intersections
            hits(2, hitsNum) = index(1);           %der kleinste Wert aller intersections 
