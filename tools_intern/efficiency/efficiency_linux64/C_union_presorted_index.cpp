...
int C_union_presorted_index(double* list_out, double* pA, double* pB, double mA, double mB, double* pIA, double* pIB, int* nIA, int* nIB)
{
	int		nOut = 0,
			iA = 0,
			iB = 0;
		...
		else 
		{
			if (pA[iA] > pB[iB])
			{
				...
				*(list_out++) = v;
				...
				while (pB[iB] == v && iB<mB) // fast forward through occurences of v
					iB++;			   
				if (pIB != NULL)
				{
					*(pIB++) = nOut;
	...
	while (iA<mA)
	{
			...
			*(list_out++) = v;
			...
			while (pA[++iA] == v); // fast forward through occurences of v
			if (pIA != NULL) // record indices AFTER previous incrementations because of Matlab indexing!!
			{
			  	    *(pIA++) = nOut;
					(*nIA)++;
	...
	while (iB<mB)
	{
			...
			*(list_out++) = v;
			...
			while (pB[++iB] == v); // fast forward through occurences of v
			if (pIB != NULL) // record indices AFTER previous incrementations because of Matlab indexing!!
			{
			  	    *(pIB++) = nOut;
					(*nIB)++;
			}
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  */
  ...
  mA = nA*mA;    //(mA>nA?mA:nA);
  mB = nB*mB;    //(mB>nB?mB:nB);

  max_list_out_length = mA+mB;  
  ...
  list_out_length = C_union_presorted_index(list_out,pA,pB,mA,mB,pIA,pIB,&nIA,&nIB);
