function [d,status,N]=XM_get_data(h, Channels, MT_BeingPlotted);
...
% Calculation of indices to be retrieved. MTObj returns ALL data from
% all MTx's connected to XM, to display data from 1 we only need a subset... 
IndicesToBeRetrieved = [(MT_BeingPlotted-1)*Channels + 1: MT_BeingPlotted * Channels];
...
for i = 1:BufferMax,
    ...
    if arg1==1,
        ...
        N = N + 1;
        d(N,1:Channels)=arg2(IndicesToBeRetrieved); % only use the first sensor attched to XM
        status=arg1;
