function varargout = features_extract(files, features_spec, varargin)
% varargout = features_extract(files, features_spec, outfile, dir_root, rle)
% 
...
t_feature = 0;
for k = 1:files_num
    file_fullpath = [dir_root files{k}];
    F.features{k} = cell(features_num,1);
    disp([num2str(k) '/' num2str(files_num) ': ' file_fullpath]);
    
    [F.features{k},F.files_sampling_rate(k),tr,tf] = features_extract_single_file(file_fullpath, features_spec, rle);
    t_read = t_read + tr;
