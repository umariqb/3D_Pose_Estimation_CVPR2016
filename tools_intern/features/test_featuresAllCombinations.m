...
M = zeros(size(combi,1),mot.nframes);
...
for r = 1:min(20,length(indices))
    p1 = combi(indices(r),1);
    p1_name = skel.nameMap{p1,1};
    p2 = combi(indices(r),2);
    p2_name = skel.nameMap{p2,1};
    p3 = combi(indices(r),3);
    p3_name = skel.nameMap{p3,1};
    q = combi(indices(r),4);
    q_name = skel.nameMap{q,1};

    s = sprintf('%4d %12.6g %10s(%2d) %10s(%2d) %10s(%2d) %10s(%2d)',r, quality_sort(r),p1_name, p1, p2_name, p2, p3_name, p3, q_name, q);
...
for k=1:p
    ...
    set(gca,'xlim',[1 mot.nframes],'ylim',[-6 6],'box','off');
%    if (k==1)
...
for k=1:p
    ...
    [x,I] = max(abs(PC(:,k)));
    plot(I,x,'red o');
    set(gca,'xlim',[1 f],'ylim',[0 0.13],'box','off');
    set(gca,'xticklabel',[]);
    set(gca,'yticklabel',[]);
end
% 
...
for e=1:p
    ...
    set(gca,'xlim',[1 mot.nframes],'ylim',[-3.5 3.5],'box','off');
    set(gca,'xticklabel',[]);
    set(gca,'yticklabel',[]);
end
