...
assert_bgl(is_kuratowski_graph(K)==1);
assert_bgl(~is_planar,'matlab_bgl:testFailed','K_5 is not planar');
assert_bgl(isequal(K,A),'matlab_bgl:testFailed','kuratowski of K_5 = A');

A = clique_graph([3,3]);
[is_planar K] = boyer_myrvold_planarity_test(A);
assert_bgl(is_kuratowski_graph(K)==1);
assert_bgl(~is_planar,'matlab_bgl:testFailed','K_3,3 is not planar');
assert_bgl(isequal(K,A),'matlab_bgl:testFailed','kuratowski of K_3,3 = A');

A = grid_graph(6,5);
[is_planar K] = boyer_myrvold_planarity_test(A);
assert_bgl(is_planar==1,'matlab_bgl:testFailed','M(6,5) is planar');
assert_bgl(isempty(K),'matlab_bgl:testFailed','no kuratowski for M(6,5)');
...
for i=1:ntrials, 
    ...
    assert_bgl(is_planar==1,'matlab_bgl:testFailed', 'Delaunay graphs are planar');
    n= size(S,1); si= zeros(size(EI.edge_order)); 
    for k=1:n, si(EI.vp(k):EI.vp(k+1)-1)=k; end
    T = sparse(si,EI.edge_order,1);
    assert_bgl(isequal(S~=0,T~=0),'matlab_bgl:testFailed', 'Incomplete Embedding Info');
    S(1:5,1:5) = clique_graph(5);
    is_planar= boyer_myrvold_planarity_test(S);    
    assert_bgl(is_planar==0,'matlab_bgl:testFailed', 'Delaunay + K5 is not planar');
end

%% is_kuratowski_graph
assert_bgl(is_kuratowski_graph(clique_graph(5))==1,'matlab_bgl:testFailed',...
    'K_5 is Kuratowski')
assert_bgl(is_kuratowski_graph(clique_graph([3,3]))==1,'matlab_bgl:testFailed',...
    'K_3,3 is Kuratowski')

rand('state',0);
for i=1:10, 
    A=sprand(50,50,5/50); A=A|A'; [isp K]=boyer_myrvold_planarity_test(A);
    if ~is_planar, assert_bgl(is_kuratowski_graph(K)==1,'matlab_bgl:testFailed',...
...
X = [0 1; 1 0];
A = clique_graph(2);
assert_bgl(is_straight_line_drawing(A,X)==1,'matlab_bgl:testFailed',...
...
if is_straight_line_drawing(A,X)==1
    ...
    assert_bgl(is_straight_line_drawing(A,X)==0,'matlab_bgl:testFailed',...
        'K_4 on a square is not a straight line drawing');
end

%% make_connected
[ei ej] = make_connected(grid_graph(5));
assert_bgl(isempty(ei) && isempty(ej), 'matlab_bgl:testFailed',...
...
for i=1:ntrials
    S= sprandsym(n,d/n);
    A= make_connected(S);
    assert_bgl(max(components(A))==1, 'matlab_bgl:testFailed',...
...
try
    ...
    assert_bgl(1,'matlab_bgl:testFailed','K5 is not planar');
catch end
...
for i=1:ntrials 
    ...
    assert_bgl(nnz(A)>=nnz(S), 'matlab_bgl:testFailed',...
        'make_biconnected_planar removed edges');
    assert_bgl(isempty(biconnected_components(A)), 'matlab_bgl:testFailed', ...
...
for i=1:ntrials, 
    ...
    assert_bgl(is_straight_line_drawing(A,X)==1,'matlab_bgl:testFailed',...
        'invalid drawing');
    assert_bgl(is_straight_line_drawing(S,X)==1,'matlab_bgl:testFailed',...
        'invalid drawing');
    assert_bgl(is_straight_line_drawing(A,Y)==1,'matlab_bgl:testFailed',...
        'invalid drawing');    
    assert_bgl(is_straight_line_drawing(S,Y)==1,'matlab_bgl:testFailed',...
        'invalid drawing');
end
