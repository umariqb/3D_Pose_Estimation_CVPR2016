...
[A,xy] = cycle_graph(0); assert_bgl(all(size(A)==[0 0]));
[A,xy] = cycle_graph(1); assert_bgl(all(size(A)==[1 1])); assert_bgl(nnz(A)==1);
...
%% star_graph
[A,xy] = star_graph(0); assert_bgl(all(size(A)==[0 0]));
[A,xy] = star_graph(1); assert_bgl(all(size(A)==[1 1])); assert_bgl(nnz(A)==0);
for i=0:10, [A,xy] = star_graph(i); end
for i=50:50:250, [A,xy] = star_graph(i); end

%% wheel_graph
[A,xy] = wheel_graph(0); assert_bgl(all(size(A)==[0 0]));
[A,xy] = wheel_graph(1); assert_bgl(all(size(A)==[1 1])); assert_bgl(nnz(A)==0);
