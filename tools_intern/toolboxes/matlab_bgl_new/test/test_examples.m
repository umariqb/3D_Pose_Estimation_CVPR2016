...
G = grid_graph(6,5);
assert_bgl(boyer_myrvold_planarity_test(G)==1,'msgid','Grid(6,5) is planar');
K5 = clique_graph(5);
assert_bgl(boyer_myrvold_planarity_test(K5)==0,'msgid','K5 is not planar');
...
% test_planar_graph
A = clique_graph(5);
assert_bgl(test_planar_graph(A)==0,msgid,'K5 is not planar');
A(4,5)=0; A(5,4)=0; % make K_5 into a planar graph
assert_bgl(test_planar_graph(A)==1,msgid,'K5-edge is planar');

% kuratowski_subgraph
A = clique_graph([3,3]); % Generate K_3,3
K = kuratowski_subgraph(A);
assert_bgl(isequal(A,K)==1,msgid,'K_3,3 is a Kuratowski graph!');

% make_maximal_planar
G = grid_graph(6,5);
M = make_maximal_planar(G);
assert_bgl(test_planar_graph(M)==1,msgid,'Maximal(Grid(6,5)) is planar');
M(9,20) = 1; M(20,9) = 1; 
assert_bgl(test_planar_graph(M)==0,msgid,'Maximal(Grid(6,5)) + edge is not planar');

% make_biconnected_planar
G = grid_graph(6,1); % generate a line graph
B = make_biconnected_planar(G);
assert_bgl(isempty(biconnected_components(B)),msgid,'make_biconnected failed');
assert_bgl(max(components(B))==1,msgid,'biconnected graphs have one component');
B(1,2) = 0; B(2,1) = 0;
assert_bgl(max(components(B))==1,msgid,'biconnected graphs - edge have one component');

% make_connected
G = sparse(2,2); % empty 2 node graph with 2 components
C = make_connected(G);
assert_bgl(max(components(C))==1,msgid,'make_connected failed');

% is_straight_line_drawing
X = [0 1; 1 0];
assert_bgl(is_straight_line_drawing(clique_graph(2),X)==1,msgid,...
    'straight line test failed');
X = [0 1; 1 0; 0 -1; -1 0];
is_straight_line_drawing(clique_graph(4),X);

% is_kuratowski_graph
assert_bgl(is_kuratowski_graph(clique_graph(4))==0,msgid,'K4 is not kuratowski');
assert_bgl(is_kuratowski_graph(clique_graph(5))==1,msgid,'K5 is kuratowski');
assert_bgl(is_kuratowski_graph(clique_graph([3,3]))==1,msgid,'K_3,3 is kuratowski');

% planar_canonical_ordering
p = planar_canonical_ordering(grid_graph(6,5));
assert_bgl(p(1)==1,msgid,'Grid(6,5) can draw vertex 1 first');
