...
for i=0:10, X=gursoy_atun_layout(sparse(i,i)); assert_bgl(check_layout(X)); end
for i=0:10, X=gursoy_atun_layout(grid_graph(i,i)); assert_bgl(check_layout(X)); end
for i=0:10, for j=2:10,
        X=gursoy_atun_layout(wheel_graph(i),'topology',sprintf('cube%i',j));
        assert_bgl(check_layout(X)); 
...
for i=0:10, X=kamada_kawai_spring_layout(grid_graph(i,i)); assert_bgl(check_layout(X)); end
