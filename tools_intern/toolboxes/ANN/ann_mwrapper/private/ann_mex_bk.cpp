...
//  Modified by Bjoern Krueger, November 2008
//
/////////////////////////////////////////////////////////////////////
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *p = (double*)mxGetPr(mxStr);
    
    //Depending on first value, call specified function:
    switch ((int)*p){
        
	case 1:{
	//Build tree
        
        const mxArray *mxPts    = prhs[1];
        ...
        // Backup pointers
        ANNkd_tree    **tmpTree = kd_tree;
        ANNpointArray  *tmpPts  = pts;

        n_trees++;
        
        // Create new arrays for pointers
        kd_tree = new ANNkd_tree*[n_trees];
        pts     = new ANNpointArray[n_trees];
        ...
        pts    [n_trees-1] = createPointArray(mxPts, d, n);        
		kd_tree[n_trees-1] = createKdTree(d, n, pts[n_trees-1], opts_build);

        // return the index of the new tree:
		plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
  		 p1 = (double*)mxGetPr(plhs[0]);
		*p1 = n_trees-1.0;
	...
	//Perform query
        // Get inputs
        const mxArray *treeInd  = prhs[1];
        const mxArray *mxPts    = prhs[2];
       ...
       // Check if specified tree exists
       if(!(kd_tree[(int)*mxGetPr(treeInd)]==NULL)){

    		performAnnkSearch(kd_tree[(int)*mxGetPr(treeInd)], mxPts, opts_search, mxInds, mxDists);
            // set output from mexFunction
            plhs[0] = mxInds;
            plhs[1] = mxDists;
       }
       else {
            mexErrMsgTxt("Specified tree doesn't exist!\n");
       }
       break;
	...
	case 3:{
	//Destroy tree
         // Parse input: which tree should be destroyed
         const mxArray *treeInd  = prhs[1];
         
         // Check if tree exists
         if(!(kd_tree[(int)*mxGetPr(treeInd)]==NULL)){
             
                delete         kd_tree[(int)*mxGetPr(treeInd)];
                kd_tree[(int)*mxGetPr(treeInd)]=NULL;

                annDeallocPts(     pts[(int)*mxGetPr(treeInd)]);
                pts[(int)*mxGetPr(treeInd)]=NULL;
         }
         
         // Set output to one for success
         plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
         p3 = (double*)mxGetPr(plhs[0]);
        *p3=1.0;
         break;
	}
    
    case 4:{
    // Final Cleanup: Destroy all trees and call annClose
        
        // Go through all trees
        for(int tree=0;tree<n_trees;tree++){
            
            //Check if current tree exists
            if(!(kd_tree[tree]==NULL)){
                
                //delete it
                delete kd_tree[tree];
                annDeallocPts(pts[tree]);
            }
        }
	...
	//Error wrong function called
		mexErrMsgTxt("Wrong option selected!\n");
        break;
	}
    }
