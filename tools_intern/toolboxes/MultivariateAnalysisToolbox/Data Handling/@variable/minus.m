function vr = minus(vr1,vr2)

% MINUS subtracts two variables
% --------------
% vr = vr1 - vr2
% --------------
...
if isnominal(vr1) || isnominal(vr2)
    error('addition is not defined for nominal variables');
end
...
mnmx1 = pminmax(vr1);
mnmx2 = pminmax(vr2);
mnmx = [mnmx1(1)-mnmx2(2) mnmx1(2)-mnmx2(1)];
mn = pmean(vr1) - pmean(vr2);

% variance and distribution are not defined
vr = variable('name',name,'description',description,'source',source,...
    'data',data,'level',level,'minmax',mnmx,'mean',mn);