function vars = gvar(varargin)

% GVAR computes intra-group variances
% ------------------------------
% vars = gvar(gr, data, h_level)
% ------------------------------
% Description: If a dataset contains {g} groups in the {h_level} hierarchy,
%              then this function computes the {g} intra-group variances.
...
% Output:      {vars} variables-by-number_of_groups computed variances.
...
vars = zeros(no_vars,no_groups);
for ii = 1:no_groups
    vars(:,ii) = ...
        var(data(:,grp2samp(gr,gr.gcn2gid{h_level}(ii),h_level)),0,2);
end
