function means = gmean(varargin)

% GMEAN computes intra-group means
% --------------------------------
% means = gmean(gr, data, h_level)
% --------------------------------
% Description: If a dataset contains {g} groups in the {h_level} hierarchy,
%              then this function computes the {g} intra-group means.
% Input:       {gr} grouping instance.
%              {data} to apply mean to. Can be either VSMATRIX, or a
%                   collection of VARIABLE objects, or a
%                   variables-by-samples matrix.
%              <{h_level}> hierarchy level (def=1).
% Output:      {means} variables-by-number_of_groups computed means.
...
means = zeros(no_vars,no_groups);
for ii = 1:no_groups
    means(:,ii) = mean(data(:,grp2samp(gr,gr.gcn2gid{h_level}(ii),h_level)),2);
end
