function [ err ] = mmwrite(filename,A,comment,field,precision)  %#ok

% MMWRITE writes a matrix in Matrix Market format.
% -----------------------------------------------------
% err = mmwrite(filename, A, comment, field, precision)
% -----------------------------------------------------
% Description: Writes the sparse or dense matrix A to a Matrix Market (MM)
%              formatted file.
% Input:       {filename} destination file.
%              {A} sparse or full matrix.
%              <{comment}> matrix of comments to prepend to the MM file.
%                   To build a comment matrix, use str2mat. For example:
%                       comment = str2mat(' Comment 1' ,...
%                                         ' Comment 2',...
%                                         ' and so on.',...
%                                         ' to attach a date:',...
%                                         [' ',date]);
%                   If ommitted, a single line date stamp comment will be
%                   included.
%              <{field}> either of 'real', 'complex', 'integer', 'pattern'.
%                   If ommitted, data will determine type.
%              <{precision}> number of digits to display for real or
%                   complex values. If ommitted, full working precision is
%                   used.
