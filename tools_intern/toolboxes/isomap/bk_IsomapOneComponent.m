function [Y, R, E] = bk_IsomapOneComponent(D, n_fcn, n_size, options) 
...
while (upK-lowK)~=1 || n_comps>1

    K = round(( lowK + upK ) / 2);
    
    fprintf(' lowK=%i K=%i upK=%i\n',lowK,K,upK);
    ...
    Dsparse = zeros(size(D));
    Dsparse(D<INF) = D(D<INF);
    Dsparse = sparse(Dsparse);
    
%     D=zeros(size(D));
    for k=1:N
        [D(k,:),~]=shortest_paths(Dsparse,k);
    end
...
%     %      D = min(D,D(:,k*ones(1,N))+D(k*ones(1,N),:));
