function [keyframes,keyframesIndex] = keyframesChoose2(motionTemplateReal,motionTemplateWeights,par)
...
%    score = (2*num_white_entries + num_black_entries)/num_features;
    scores(i,:)  = (2*num_white_entries(i,:) + 1.2*num_black_entries(i,:))/num_features;
...
%     drawnow;
    ...
    I_left_0 = max(I_frame(I_feature_set)-par.keyframeMinDist,1);
    I_right_0 = min(I_frame(I_feature_set)+par.keyframeMinDist,num_frames);
    I_left = I_left_0;
    I_right = I_right_0;
    ...
    scores_help(:,I_left_0:I_right_0) = 0;
    scores_help(ind,I_left:I_right) = 0;
%    scores_help(:,I_left:I_right) = 0;
end
