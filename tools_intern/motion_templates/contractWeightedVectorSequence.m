function [V_out,weights_out] = contractWeightedVectorSequence(V_in,weights_in)
...
while counter_in <= n
    ...
    if s>=0.75
        ...
        V_out(:,counter_out) = sum(repmat(weights_in(counter_in:counter_right),dimV,1).*V_in(:,counter_in:counter_right),2) / s;
        weights_out(counter_out) = s;
